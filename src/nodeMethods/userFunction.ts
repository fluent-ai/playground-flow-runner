...
async function runUserScript(userScript: string, context: Record<string, unknown>) {
  // Check for disallowed words
  const disallowedWords = ['import', 'require', 'eval', 'window', 'alert', 'document']
...
export async function userFunction(
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      runUserScript(data.userFunction as string, { msg, data }).then(result => {
        resolve({ ...msg, ...result })
      })
    } catch (error) {
      reject(error)
